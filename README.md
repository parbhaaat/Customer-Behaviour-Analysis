...
│
├── python/
│   └── customer_eda_cleaning.ipynb          # EDA & data cleaning notebook
│
├── sql/
│   └── business_queries.sql                 # All 12 SQL queries
│
├── dashboard/
│   └── customer_insight_engine.pbix         # Power BI dashboard file
│
├── screenshots/
│   ├── dashboard_page1.png
│   └── dashboard_page2.png
...
1. **Clone the repo**
   ```bash
   git clone https://github.com/yourusername/customer-insight-engine.git
   ```

2. **Run the Python notebook**
   - Open `python/customer_eda_cleaning.ipynb`
   - Install dependencies: `pip install pandas pyodbc sqlalchemy`
   - Run all cells — this cleans the data and exports to SQL Server

3. **Run SQL queries**
   - Open `sql/business_queries.sql` in SQL Server Management Studio (SSMS)
   - Execute queries against the imported table

4. **Open the dashboard**
   - Open `dashboard/customer_insight_engine.pbix` in Power BI Desktop
